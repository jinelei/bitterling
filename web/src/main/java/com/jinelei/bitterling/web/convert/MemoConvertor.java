...
public interface MemoConvertor {
    ...
    default MemoDomain.DetailResponse transTags(@MappingTarget MemoDomain.DetailResponse source,
        ...
        final List<Long> tagIds = Optional.ofNullable(tags).map(l -> l.stream().map(RecordDomain::getId).toList())
                .orElse(new ArrayList<>());
        return new MemoDomain.DetailResponse(source.id(), source.title(), source.subTitle(), source.content(),
                source.contentRender(), tags, tagIds, source.orderNumber(), source.createTime(), source.updateTime());
    }
    ...
    default MemoDomain.DetailResponse transContentRender(@MappingTarget MemoDomain.DetailResponse source) {
        final String contentRender = Optional.ofNullable(source)
                ...
                .orElse("");
        return new MemoDomain.DetailResponse(source.id(), source.title(), source.subTitle(), source.content(),
                contentRender, source.tags(), source.tagIds(), source.orderNumber(), source.createTime(),
                source.updateTime());
    }
